...
| `.gitignore` | Excludes `.env`, `venv/`, `__pycache__/`, `bot/.env`, `.Codex/` |
| `main.py` | Fetches 1-year stock data for a watchlist, sends to Codex, saves text report to `analysis.txt` |
...
| `app.py` | Simpler Flask variant that asks Codex for action ratings on a stock list |
...
| `strategy.py` | Codex-powered signal scoring and sell detection; defines the 24-ticker WATCHLIST (Canadian ETFs, Canadian stocks, US growth, US ETFs); `get_sell_signals()` returns urgency: URGENT / WARNING / FYI / None |
...
| AI analysis | Anthropic Codex (Codex-sonnet-4-6) via `anthropic` SDK |
...
| Variable | Purpose |
|----------|---------|
| `ANTHROPIC_API_KEY` | Codex API access |
